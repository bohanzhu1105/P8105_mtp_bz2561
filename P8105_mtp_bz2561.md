...
After importing from Excel, the dataset was cleaned by skipping unused
header rows, standardizing variable names, converting missing values to
NAs, and recoding numeric codes into clear labels. `Sex` was relabeled
as male/female, and `age groups` as 18–30, 31–40, etc.
...
**I assume cleaning rule for FHP size is same as EOP size** After
cleaning, the resulting dataset contained 1221 observations and 9
variables. Among the variables, several are key to the analysis.

`EOP size` and `eop_visibility_classification` are crucial because they
capture the prominence of the external protuberance. `FHP size` and
`FHP category` are also important, as they measure forward head posture
potentially linked to EOP growth. Finally, `sex` and `age group` offer
demographic context for biological or behavioral differences.
